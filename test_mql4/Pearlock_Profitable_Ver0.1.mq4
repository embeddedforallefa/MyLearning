...
//---------------------------------------------------------------------
   ...
   if(res == -1)
   ...
   else
     {
      ...
      for(int j=0; j < licence_len; j++)
           {
            if((j%2)==0)
            {
               if(result[j]==licence_local[j]+1)
               {
                  licence_valid = true;
               }
               else
...
//---------------------------------------------------------------------
   ...
   IndicatorShortName(timeFrameToString(TimeFrame)+" Profitable("+(string)OmaLength+","+(string)Sensitivity+","+(string)StepSize+")");
   return(0);
}
int deinit() { return(0); }     
   ...
   if (TimeFrame == Period())
   {
      ...
      for(i=limit; i>=0; i--)
      {
         ...
         int fperiod = OmaLength; if (FilterPeriod>0) fperiod=FilterPeriod;
   	   ...
   	   LineBuffer[i] = iProfitable(Sensitivity,iATR(NULL,0,StepSize,i),1.0,thigh,tlow,Close[i],i);
...
double iProfitable(double sensitivity, double stepSize, double stepMulti, double phigh, double plow, double pprice, int r)
{
